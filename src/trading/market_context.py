...
Airflow가 수집한 market_data 테이블에서 최신 VKOSPI / KOSPI 12이평 위치를
live_trader가 읽어 장중 레짐을 동적으로 갱신하는 브릿지 역할을 한다.
...
class MarketContext:
    ...
    Airflow 수집 market_data 테이블에서 VKOSPI / KOSPI 12이평 위치를 읽어
    레짐 판단에 필요한 최신 값을 제공한다.
    ...
    async def refresh(self) -> None:
        ...
        try:
            ...
            log.info(
                "MarketContext 갱신 완료: VKOSPI=%.1f, KOSPI>12이평=%s",
                self._vkospi,
    ...
    async def _fetch_from_db(self) -> None:
        """market_data 테이블에서 최신 VKOSPI / kospi_regime 레코드를 조회한다.
