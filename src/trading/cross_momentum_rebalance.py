...
class CrossMomentumRebalanceAdapter:
    ...
    async def compute_target_portfolio(self, today: date) -> list[str]:
        ...
        # stale guard (PR A + PR A2): 검증 시점 기준 마지막으로 완성된 일봉 날짜.
        # 장중 호출 (예: 14:55 weekly trigger) 시에는 직전 영업일이 cutoff — 오늘 일봉이
        # 아직 채워지지 않은 상태를 stale 로 오판하지 않게 한다. 장 후 호출 시에는 today.
        require_fresh_through = latest_completed_business_day(today)
...
def _is_rebalance_trigger_date(today: date, freq: str) -> bool:
    """오늘이 cross_momentum 리밸런스 trigger date인지 판정.

    F.8: ``rebalance_freq`` 의 단일 진실원은 DB ``strategy_config`` 의
    ``cross_momentum.rebalance_freq``. env ``CROSS_MOMENTUM_REBALANCE_FREQ`` 는
    deprecate 됐다. 호출처가 DB 에서 읽은 값을 명시 인자로 전달해야 한다.
    ...
    Args:
        today: KST date
        freq: 리밸런스 주기 (DB strategy_config 에서 로드). monthly 또는 weekly.
...
async def check_monthly_rebalance(
    ...
    조건:
      ...
      - today가 trigger date (DB strategy_config.cross_momentum.rebalance_freq 기준)
        - monthly: 매월 마지막 영업일
        - weekly: 매주 금요일 영업일
