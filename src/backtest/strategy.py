...
class MomentumParams:
    ...
    volume_ratio: float = 0.5  # 거래량 배수 (공격적: 0.5x, 보수적: 0.7x)
    stop_loss: float = -0.015  # -1.5% (공격적: 넓은 손절폭으로 조기 청산 방지)
    take_profit: float = 0.03  # +3.0% (R:R 1:2)
    trailing_stop_pct: float | None = -0.01  # 고점 대비 -1.0% (공격적: 넓은 트레일링)
    max_positions: int = 5  # 분산 투자 (전략당 5, 총합 10)
    high_52w_threshold: float = 0.0  # 0이면 비활성 (단타 모드)
    price_change_min: float = 0.003  # 당일 시가 대비 최소 상승률 (0.3%, 완화)
    ...
    entry_end_time: str = "15:00"  # 오후 3시까지 진입 (마감 직전까지 포착)
