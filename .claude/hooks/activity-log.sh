...
if [[ "$TOOL_NAME" == "Write" || "$TOOL_NAME" == "Edit" || "$TOOL_NAME" == "NotebookEdit" ]]; then
  FILE_PATH=$(echo "$INPUT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('file_path', d.get('notebook_path','')))" 2>/dev/null || echo "unknown")
...
import sys, json, re
cmd = json.load(sys.stdin).get('command', '')[:200]
# 민감 정보 마스킹
