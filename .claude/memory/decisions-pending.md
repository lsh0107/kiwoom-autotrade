...
> **마지막 검토**: 2026-03-14
> **상태**: 4건 미결정 (#14,15,17,19,20), 5건 신규 확정 (#11,12,13,16,18). 새 미결정 사항 발생 시 이 문서에 추가할 것.
...
### 16. 쿨다운 메커니즘 파라미터
- **배경**: 2026-03-13 모의실전에서 같은 종목 손절 즉시 재매수 반복 → "고빈도 손절 머신" 패턴 발생
- **선택지**:
  - A) 종목별 매도 후 30분 재진입 금지 + 연속 손절 3회 당일 블랙리스트 ← **design-strategy-v2.md 권장**
  - B) 종목별 매도 후 60분 + 연속 손절 2회 블랙리스트 (더 보수적)
  - C) 전체 연속 손절 5회 시 30분 전면 매수 중단 (추가 안전장치)
- **트레이드오프**: 기회 비용 (유효 매수 기회 놓침) vs 손실 방지 (반복 손절 차단)
- **결정 시점**: Phase 1 패치 즉시 A안 적용 후, 1주일 모의실전 데이터로 튜닝
- **관련**: `design-strategy-v2.md` Phase 1-3
...
### 18. 테마/섹터 분류 방식
- **배경**: 동일 섹터 종목 동시 보유 = 분산 아닌 집중 리스크. 조선주 3개/금융주 4개 동시 보유로 손실 증폭
- **선택지**:
  - A) 하드코딩 SECTOR_MAP (50종목 수동 분류) ← **Phase 1 즉시 적용 가능**
  - B) GICS 코드 기반 자동 분류 (키움 API ka10017 업종 정보)
  - C) LLM 기반 테마 추출 (뉴스 + 공시 → 동적 테마)
- **트레이드오프**: A는 즉시 가능하나 유니버스 변경 시 수동 갱신 필요. B는 공식 분류이나 테마(ex: AI, 방산)를 못 잡음. C는 최유연하나 Phase 3 의존
- **규칙**: 같은 섹터 최대 1개 포지션
- **결정 시점**: Phase 1에서 A안 즉시 적용, Phase 3에서 C안으로 점진 전환

### 19. 스윙 전략 overnight 보유 허용 범위
- **배경**: 현재 모든 포지션 장 종료 시 강제 청산. 스윙 전략은 overnight 보유가 핵심
- **선택지**:
  - A) 스윙 전략만 overnight 허용, 최대 5거래일 ← **design-strategy-v2.md 권장**
  - B) 전 전략 당일 청산 유지 (안전)
  - C) overnight 허용하되 갭 리스크 헤지 (선물 헤지 등 — 모의투자 미지원)
- **하위 결정**: 갭다운 손절 기준 (-3%?), 보유기간 제한 (5일?), 포지션 복원 로직
- **트레이드오프**: 수익 기회(추세 포착) vs 갭 리스크(통제 불가)
- **결정 시점**: Phase 2, 단타 전략 안정화 후
