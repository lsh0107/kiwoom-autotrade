...
> **상태**: 참조 (리서치 완료, 결정 반영됨)
>
> **최종 결정: 키움 REST API를 Primary로 선택 (ADR-001)**
> - 리서치 당시 "KIS API 추천"이었으나, 사용자가 키움증권 계좌를 보유 + 키움 국내 점유율 1위 + 국내주식만 거래할 것이므로 키움으로 결정
> - 이 문서의 섹션 6 결론은 리서치 시점의 일반적 추천이며, 프로젝트 실제 결정과 다름
> - 향후 해외주식 필요 시 KIS API 추가 검토 (BrokerClient Protocol로 교체 가능하게 설계됨)
>
> **적용된 결정 (ADR-021)**: KIS→키움 마이그레이션 완료 (2026-03-05)
> - `src/broker/` 코드가 실제 키움 REST API 형식으로 전환됨
> - KIS API 형식 임시 구현 → 키움 REST API 형식 (api-id, 전부 POST, KRX:종목코드 등)
...
| WebSocket | O | O | ? |
| 모의투자 | O | O (무료) | O |
| REST Rate | 초당 ~20 (추정, 커뮤니티 기반) | 초당 20 (실전) / 5 (모의) | 미공개 |
