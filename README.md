...
| 022 | [design-022](docs/design/design-022-cross-momentum-live-adapter.md) | Cross-momentum live rebalance 어댑터 — CrossMomentumRebalanceAdapter, 월말 14:55 스케줄러, USE_CROSS_MOMENTUM 환경변수 |
...
**전략 검증 현황 (2026-04-28)**: 누적 폐기 5건. **ADR-021 cross-sectional momentum V2 기준 PASS** (top20pct_novol_notrend, 33%). **ADR-022 어댑터 + ADR-023 견고화 완료** (rate limit 백오프 + T+2 결제 + KRX 공휴일, 1871 PASS). `USE_CROSS_MOMENTUM=true` 설정 후 모의 4주 관찰 시작 가능. 실전 전환은 모의 4주 기준 통과 + 사용자 명시적 승인 필수.
