...
| 2026-04-27 | 전 전략 0/59 폐기 확정 — **일봉 timeframe** 한정 폐기 (주봉~월봉은 별개 검증 대상 보존) |
...
| 5 | Pullback/Range/MR **일봉(daily) timeframe** (KOSPI30+KOSDAQ30 59종목, 3년) | 27 combo × 59종목 0/59 — 거래 발생(OOS 31%)해도 수익 불가. **주봉~월봉 timeframe은 별개 검증 대상으로 보존** | **ADR-020** |
...
3. **일봉(daily) timeframe에서의 mean-reversion/range/pullback 부적합성**: KOSPI·KOSDAQ를 불문하고, 3년·59종목·27 combo 전부 실패는 개별 종목·파라미터 문제가 아니라 일봉 timeframe + 이 전략 카테고리 조합의 구조적 한계임을 의미한다. **단, 주봉/이주봉/월봉 timeframe은 노이즈/거래비용 비율이 일봉과 완전히 달라 본 ADR의 결론으로 폐기되지 않으며, 별개 검증 대상으로 보존한다.**

---

## 6. 핵심 결정

**일봉(daily) timeframe에서의 Pullback / Range / MR (mean-reversion) 카테고리를 한국 시장 검증 대상에서 폐기한다.**

| 근거 | 상세 |
|------|------|
| 신호 희소성 아님 | OOS 31% 윈도우에서 거래 발생 확인 |
| 일봉 카테고리 수익 불가 | 27 combo × 59종목 × 3년 전부 기준 미달 |
| 파라미터 탐색 소진 | ADR-019에서 이미 grid 확장 시도 — ADR-020에서 확인 |
| 투자 비용 대비 효과 없음 (일봉 한정) | 일봉 추가 검증은 동일한 결론의 반복에 불과 |

**범위 명시**: 본 결정은 **일봉(daily) timeframe**에 한한다. 주봉(weekly), 이주봉(bi-weekly), 월봉(monthly) timeframe에서의 동일 전략 카테고리는 노이즈/거래비용 비율이 본질적으로 다르므로 별도 검증 시 부활 가능하다 (옵션 (e) 참조).
...
| **(e) 주봉/이주봉/월봉 timeframe 재검증** | Pullback/Range/MR/모멘텀을 일봉 → 주봉~월봉으로 변환 후 walk-forward 재실행. 노이즈 감소 + 거래횟수 1/5~1/20 → 왕복비용 영향 감소. pykrx 일봉을 resample 가능하므로 데이터 수집 불필요 | 낮음 (엔진 재활용 + 리샘플 레이어만 추가) | 중간 — 시계열 길이 단축으로 윈도우 수 감소 우려 |
...
| 일봉 timeframe | Pullback/Range/MR **일봉** 재검증 금지 — 추가 검증은 동일 결론의 반복 |
| 주봉~월봉 timeframe | 본 ADR로 폐기되지 않음. 옵션 (e)로 별도 검증 가능 (USE_MULTI_REGIME flag 별도 확장 필요) |
